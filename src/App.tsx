...
import {
  ...
  Edit3, Plus, Trash2, Clock, BellRing
} from 'lucide-react';
...
interface UserState {
  ...
  notifHour: number; // час уведомления (по умолч. 19)
}
...
// =============================================
// УВЕДОМЛЕНИЯ БЕЗ БЭКЕНДА
// =============================================

// Глобальный ID таймера
let notifTimeoutId: ReturnType<typeof setTimeout> | null = null;

/**
 * Планирует уведомление на указанный час.
 * Если юзер уже зашёл сегодня — не шлёт, но планирует на завтра.
 * Работает через setTimeout + Service Worker showNotification.
 */
function scheduleNotification(notifHour: number = 19) {
  ...
  // Цель: сегодня в notifHour:00 или завтра
  ...
  console.log(`[BEMAT] Уведомление запланировано через ${Math.round(delay / 60000)} мин (${target.toLocaleString()})`);

  notifTimeoutId = setTimeout(async () => {
    // Проверяем: юзер заходил сегодня?
    ...
    if (saved) {
      try {
        ...
        if (lastVisit.toDateString() === today.toDateString()) {
          // Юзер уже заходил сегодня — не шлём, но планируем на завтра
          console.log('[BEMAT] Юзер уже заходил сегодня, пропускаем');
          scheduleNotification(parsed.notifHour || 19);
          return;
        }
      } catch { /* ignore */ }
    }
    ...
    // Шлём уведомление!
    const messages = [
      { title: 'Боб проголодался! 🐱', body: 'Зайди позаниматься, чтобы покормить Боба и не потерять стрик 🔥' },
      { title: 'Не забудь про английский! 📚', body: 'Всего 15 минут — и Боб будет сытым и довольным 😸' },
      { title: 'Стрик горит! 🔥', body: 'Зайди на 15 минут, чтобы сохранить свою серию дней' },
      { title: 'Боб скучает! 😿', body: 'Он ждёт тебя, чтобы вместе поучить английский' },
    ];
    const msg = messages[Math.floor(Math.random() * messages.length)];

    try {
      // Пробуем через Service Worker (работает в фоне)
      ...
      if (reg) {
        ...
        new Notification(msg.title, { body: msg.body, icon: '/logo192.png' });
      }
    } catch {
      try { new Notification(msg.title, { body: msg.body }); } catch { /* */ }
    }

    console.log('[BEMAT] Уведомление отправлено!');
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-[#fafaf9] rounded-[2rem] shadow-2xl p-8 border border-white text-center">
        ...
        {isFirst ? (
          ...
          <>
            <h3 className="text-2xl font-black text-stone-900 mb-2">{streak} {streak < 5 ? 'дня' : 'дней'} подряд!</h3>
        ...
        <div className="bg-violet-50 rounded-xl p-3 mb-6 border border-violet-100">
          <p className="text-xs font-bold text-violet-700">
            {isFirst ? '💡 Совет: занимайся хотя бы 15 минут в день!'
              : streak >= 7 ? '🏆 Неделя без пропусков! Ты — машина!'
              : streak >= 3 ? '💪 3+ дня подряд — отличное начало!'
      ...
      <div className="relative w-full max-w-sm bg-[#fafaf9] rounded-[2rem] shadow-2xl p-6 border border-white text-center">
        ...
        {isFed ? (
          <>
            ...
            <p className="text-stone-500 text-sm mb-6">Приходи завтра 😸</p>
          </>
        ) : (
          <>
            <div className="text-4xl mb-2">😿</div>
            ...
            <p className="text-xs text-stone-400 mb-6">Осталось: {remaining} {remaining === 1 ? 'задание' : remaining < 5 ? 'задания' : 'заданий'}</p>
          </>
        )}

        <div className="mb-2">
          <div className="flex items-center justify-between text-xs font-bold text-stone-500 mb-1.5">
            <span>🍽️ Миска с кормом</span><span>{feedProgress}%</span>
          </div>
...
const ProfileModal = ({ isOpen, onClose, user, onSave }: { isOpen: boolean; onClose: () => void; user: UserState; onSave: (name: string, goal: UserGoal) => void }) => {
  ...
  useEffect(() => { if (isOpen) { setEditName(user.name); setSelectedGoal(user.goal); } }, [isOpen, user]);

  const handleSave = () => { if (!editName.trim()) return; onSave(editName.trim(), selectedGoal); onClose(); };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#fafaf9] rounded-[2rem] shadow-2xl p-6 border border-white max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-center mb-6">
          <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-white shadow-lg"><img src={logo} alt="Bob" className="w-full h-full object-cover" /></div>
        </div>
        <div className="mb-6">
          <label className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-2 block">Имя</label>
          <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full p-3.5 rounded-xl bg-white border border-stone-200 text-base font-bold focus:outline-none focus:ring-2 focus:ring-violet-500 shadow-sm" />
        </div>
        <div className="mb-6">
        ...
        <button onClick={handleSave} disabled={!editName.trim()} className="w-full py-3.5 bg-violet-600 text-white font-bold rounded-2xl hover:bg-violet-700 shadow-lg shadow-violet-200 disabled:opacity-50">Сохранить</button>
      </div>
    </div>
  );
};
...
const AddTaskModal = ({ isOpen, onClose, onAdd }: { isOpen: boolean; onClose: () => void; onAdd: (title: string, time: number) => void }) => {
  ...
  const handleAdd = () => { if (!title.trim()) return; onAdd(title.trim(), time); setTitle(''); setTime(5); onClose(); };
  if (!isOpen) return null;
      ...
      <div className="relative w-full max-w-md bg-[#fafaf9] rounded-[2rem] shadow-2xl p-6 border border-white">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Например: Посмотреть TED Talk"
          ...
          <div className="bg-stone-50 rounded-xl p-3 border border-stone-100">
            <p className="text-xs text-stone-500">💡 Задание будет появляться <strong>каждый день</strong>. Удалить можно в любой момент.</p>
          </div>
          <button onClick={handleAdd} disabled={!title.trim()} className="w-full py-3.5 bg-violet-600 text-white font-bold rounded-2xl disabled:opacity-50 hover:bg-violet-700 shadow-lg shadow-violet-200">Добавить ✅</button>
        </div>
      </div>
    </div>
  );
...
// --- НАСТРОЙКИ УВЕДОМЛЕНИЙ (попап) ---

const NotifSettingsModal = ({ isOpen, onClose, user, onSave }: {
  isOpen: boolean; onClose: () => void; user: UserState;
  ...
  const toggleEnabled = async () => {
    if (!enabled) {
      // Включаем — запрашиваем разрешение
      if (!("Notification" in window)) { alert("Браузер не поддерживает уведомления"); return; }
      if (Notification.permission === 'denied') { alert("Уведомления заблокированы. Разреши их в настройках браузера."); return; }
    }
    setEnabled(!enabled);
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-[#fafaf9] rounded-[2rem] shadow-2xl p-6 border border-white">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-stone-900">Напоминания</h3>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-stone-200 text-stone-500"><X size={20} /></button>
        </div>
        ...
        <div className="text-center mb-6">
          <div className="w-20 h-20 mx-auto rounded-full overflow-hidden border-4 border-white shadow-lg mb-3">
            <img src={catHungry} alt="Bob" className="w-full h-full object-cover" />
          </div>
          <p className="text-sm text-stone-600">Боб будет напоминать тебе заниматься, если ты забудешь зайти</p>
        </div>
        ...
        <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-stone-100 mb-4">
          ...
          <button onClick={toggleEnabled}
            className={cn("w-12 h-7 rounded-full transition-colors relative", enabled ? "bg-violet-600" : "bg-stone-200")}>
            <div className={cn("w-5 h-5 bg-white rounded-full absolute top-1 transition-all shadow-sm", enabled ? "right-1" : "left-1")} />
          </button>
        </div>
        ...
        {enabled && (
          <div className="mb-6">
            <label className="text-xs font-bold text-stone-500 uppercase mb-2 block">Во сколько напоминать?</label>
            <div className="grid grid-cols-4 gap-2">
              {[9, 12, 15, 17, 18, 19, 20, 21].map(h => (
                <button key={h} onClick={() => setHour(h)}
                  className={cn("py-2.5 rounded-xl font-bold text-sm border-2 transition-all",
                    hour === h ? "border-violet-600 bg-violet-50 text-violet-700" : "border-stone-100 bg-white text-stone-600"
                  )}>{h}:00</button>
              ))}
            </div>
            <p className="text-[11px] text-stone-400 mt-2">⏰ Уведомление придёт если ты не заходил в этот день</p>
          </div>
        )}

        <button onClick={handleSave} className="w-full py-3.5 bg-violet-600 text-white font-bold rounded-2xl hover:bg-violet-700 shadow-lg shadow-violet-200">
          {enabled ? '✅ Сохранить' : 'Сохранить'}
        </button>
      </div>
    </div>
  );
...
const Dashboard = ({ user, onUpdateUser, onNavigate }: { user: UserState; onUpdateUser: (u: UserState) => void; onNavigate: (tab: string) => void }) => {
  ...
  const toggleTask = (taskId: string) => {
    ...
    const newCompleted = isCompleted ? user.completedTasks.filter(id => id !== taskId) : [...user.completedTasks, taskId];
    const newValid = newCompleted.filter(id => allTasks.some(t => t.id === id));
    const allDone = newValid.length >= allTasks.length;
    if (allDone && !user.catFed) setTimeout(() => setShowCatPopup(true), 500);
  ...
  const handleTaskLink = (e: React.MouseEvent, task: any) => {
    ...
    if (['home', 'books', 'video', 'practice', 'speak'].includes(hash)) onNavigate(hash);
    else { onNavigate('home'); setTimeout(() => document.getElementById(hash)?.scrollIntoView({ behavior: 'smooth' }), 300); }
  };

  const addCustomTask = (title: string, time: number) => {
    onUpdateUser({ ...user, customTasks: [...(user.customTasks || []), { id: `custom_${Date.now()}`, title, time }] });
  };

  const deleteCustomTask = (taskId: string) => {
    onUpdateUser({ ...user, customTasks: (user.customTasks || []).filter(t => t.id !== taskId), completedTasks: user.completedTasks.filter(id => id !== taskId), catFed: false });
  };

  const handleProfileSave = (name: string, goal: UserGoal) => {
    const goalChanged = goal !== user.goal;
    onUpdateUser({ ...user, name, goal, completedTasks: goalChanged ? [] : user.completedTasks, catFed: goalChanged ? false : user.catFed });
  };

  const handleNotifSave = (enabled: boolean, hour: number) => {
    const updated = { ...user, notificationsEnabled: enabled, notifHour: hour };
    onUpdateUser(updated);
    if (enabled) {
      scheduleNotification(hour);
      // Тестовое подтверждение
      if (Notification.permission === 'granted') {
        new Notification('Напоминалки включены! 🔔', {
          body: `Буду напоминать в ${hour}:00, если ты не зайдёшь`,
          icon: '/logo192.png',
        });
      }
    } else {
  ...
  return (
    <div className="pb-24 pt-4 px-4 space-y-5">
      ...
      <div className="flex justify-between items-center">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowProfileModal(true)} className="p-1.5 rounded-full hover:bg-stone-100 text-stone-400 hover:text-violet-600"><Edit3 size={16} /></button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-stone-100 relative overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-5 mt-2">
          <div>
            ...
            <p className="text-stone-500 text-xs flex items-center gap-1.5"><Clock size={12} /> ~{totalTime} мин · Сделано: {doneTime}/{totalTime} мин ({progress}%)</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {defaultTasks.map(task => {
            ...
            return (
              <div key={task.id} className={cn("flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer", isDone ? "bg-stone-50 border-transparent opacity-60" : "bg-white border-stone-100 hover:border-violet-200 shadow-sm")} onClick={() => toggleTask(task.id)}>
                <div className={cn("w-6 h-6 rounded-full flex items-center justify-center border shrink-0", isDone ? "bg-violet-500 border-violet-500" : "border-stone-300")}>{isDone && <CheckCircle2 className="w-4 h-4 text-white" />}</div>
                <div className="flex-1 min-w-0">
                ...
                {!isDone && task.link && <button onClick={(e) => handleTaskLink(e, task)} className="p-2 text-violet-600 bg-violet-50 hover:bg-violet-100 rounded-lg shrink-0"><ArrowRight size={16} /></button>}
              </div>
            );
          })}
          ...
          {customDailyTasks.length > 0 && (
            <div className="pt-2 border-t border-dashed border-stone-200">
              ...
              {customDailyTasks.map((task: any) => {
                const isDone = validCompleted.includes(task.id);
                return (
                  <div key={task.id} className={cn("flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer mb-2 last:mb-0", isDone ? "bg-stone-50 border-transparent opacity-60" : "bg-violet-50/50 border-violet-100 hover:border-violet-200 shadow-sm")} onClick={() => toggleTask(task.id)}>
                    <div className={cn("w-6 h-6 rounded-full flex items-center justify-center border shrink-0", isDone ? "bg-violet-500 border-violet-500" : "border-violet-300")}>{isDone && <CheckCircle2 className="w-4 h-4 text-white" />}</div>
                    <div className="flex-1 min-w-0">
                      <span className={cn("font-bold text-sm text-stone-700 block", isDone && "line-through text-stone-400")}>{task.title}</span>
                      <span className="text-[11px] text-stone-400">~{task.time} мин · своё</span>
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); deleteCustomTask(task.id); }} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg shrink-0"><Trash2 size={14} /></button>
                  </div>
                );
              })}
            </div>
        ...
        <button onClick={() => setShowAddTask(true)} className="mt-4 w-full py-3 bg-stone-50 hover:bg-stone-100 border-2 border-dashed border-stone-200 rounded-xl text-sm font-bold text-stone-500 hover:text-violet-600 flex items-center justify-center gap-2"><Plus size={18} /> Добавить своё задание</button>

        {progress === 100 && <div className="mt-4 p-3 bg-green-50 text-green-700 rounded-xl text-sm font-bold text-center">Все задания выполнены! Боб покормлен 😸🎉</div>}
      </div>

      {/* УВЕДОМЛЕНИЯ — новый блок */}
      <div onClick={() => setShowNotifSettings(true)}
        className={cn("rounded-[2rem] p-5 shadow-sm border cursor-pointer transition-all hover:shadow-md",
        ...
        <div className="flex items-center gap-4">
          <div className={cn("w-12 h-12 rounded-full flex items-center justify-center shrink-0",
            ...
            {user.notificationsEnabled ? <BellRing size={24} className="text-violet-600" /> : <Bell size={24} className="text-white" />}
          </div>
          <div className="flex-1">
            {user.notificationsEnabled ? (
              <>
                <h3 className="font-bold text-violet-900 text-sm">Напоминания включены ✅</h3>
                <p className="text-xs text-violet-600">Каждый день в {user.notifHour || 19}:00 · Нажми чтобы настроить</p>
              </>
            ) : (
              <>
                <h3 className="font-bold text-white text-sm">Включить напоминания</h3>
                <p className="text-xs text-violet-100">Боб напомнит позаниматься, чтобы не потерять стрик</p>
              </>
            )}
          </div>
        </div>
      ...
      <NotifSettingsModal isOpen={showNotifSettings} onClose={() => setShowNotifSettings(false)} user={user} onSave={handleNotifSave} />
    </div>
  );
};
  ...
  return (
    <>
          ...
          <div className="w-80 h-full bg-[#fafaf9] p-6 shadow-2xl overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2 mb-6">
              ...
              {[
                ...
                <a key={b.l} href={b.u} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between px-3 py-3 rounded-xl hover:bg-violet-50 group border border-transparent hover:border-violet-100">
      ...
      <Modal isOpen={isAboutOpen} onClose={() => setIsAboutOpen(false)} title="О приложении BEMAT">
        <div className="space-y-4">
          <p className="text-stone-700"><strong>BEMAT</strong> — быстрые ссылки на лучшие бесплатные сервисы для изучения английского.</p>
      ...
      <Modal isOpen={isSupportOpen} onClose={() => setIsSupportOpen(false)} title="Поддержать проект">
        <div className="text-center">
          ...
          <p className="text-stone-600 mb-6 text-sm">BEMAT — бесплатный проект. Ваша поддержка помогает Бобу!</p>
          <div className="space-y-3">
...
const HomePanel = ({ onNavigate }: { onNavigate: (tab: string) => void }) => {
  const CARDS = [
    { title: "Бесплатные курсы", desc: "Курсы по английскому в одном месте", icon: GraduationCap, color: "text-blue-500", action: () => document.getElementById('courses')?.scrollIntoView({ behavior: 'smooth' }) },
    { title: "Боты и материалы", desc: "Боты для экзаменов и практики", icon: Bot, color: "text-violet-500", action: () => document.getElementById('bots')?.scrollIntoView({ behavior: 'smooth' }) },
    ...
    { title: "Грамматика", desc: "150+ уроков грамматики", icon: PenTool, color: "text-amber-500", action: () => onNavigate('practice') },
  ...
  return (
    <div className="pb-24 space-y-8 pt-4 px-4">
      <div className="grid grid-cols-2 gap-3">
        {CARDS.map(c => (
          <div key={c.title} className="bg-white p-4 rounded-2xl border border-stone-100 shadow-sm hover:shadow-md flex flex-col items-start gap-3">
            ...
            <button onClick={c.action} className="mt-2 w-full py-2.5 bg-violet-600 text-white rounded-xl text-xs font-bold hover:bg-violet-700 shadow-sm shadow-violet-200 active:scale-95">Перейти</button>
          </div>
        ))}
      </div>
      ...
      <div id="courses" className="scroll-mt-24">
        ...
        <MediaRow title="Плейлист‑курсы в TG" desc="Подборка курсов под каждый уровень." img="https://static.tildacdn.info/tild3534-3233-4463-a134-346339623162/7A7E2857-CCF4-42C5-A.jpeg" link="https://t.me/to_be_eng/190" btnText="Открыть" />
      ...
      <div id="bots" className="scroll-mt-24">
        ...
        <MediaRow title="ЕГЭ Английский с ИИ" desc="План, стратегии, Speaking. 80+ баллов" img="https://bemat.ru/egeai.jpg" link="https://t.me/EGE_ENGLISH_GPT_bot" btnText="Попробовать" />
        <MediaRow title="ОГЭ Английский с ИИ" desc="План, стратегии, Speaking." img="https://bemat.ru/ogeai.jpg" link="https://t.me/OGE_ENG_HELPER_BOT" btnText="Попробовать" />
        <MediaRow title="IELTS эксперт" desc="Academic/General: стратегия." img="https://static.tildacdn.info/tild3532-3932-4635-a261-306563383261/11.jpg" link="https://t.me/IELTS_berdiyev_bot" btnText="Попробовать" />
        <MediaRow title="TOEFL iBT эксперт" desc="План, практика, критерии." img="https://static.tildacdn.info/tild3936-3366-4461-a139-656230353061/10.jpg" link="https://t.me/TOBEENG_TOEFL_IBT_BOT" btnText="Попробовать" />
        <MediaRow title="ЕГЭ материалы" desc="Лексика, грамматика, шаблоны." img="https://bemat.ru/egemat.jpg" link="https://t.me/tobeeng_ege_bot" btnText="Открыть" />
        <MediaRow title="Боб — Английский с ИИ" desc="Заговори за 3 месяца" img="https://bemat.ru/bobai.jpg" link="https://t.me/Tobeeng_GPT_bot" btnText="Попробовать" />
      </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-stone-200 shadow-lg">
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1">
              ...
              <p className="text-stone-600 leading-relaxed mb-4">Я — Абдуррахим Бердиев. Помогаю заговорить на английском. Готовлю к ЕГЭ/ОГЭ и IELTS/TOEFL.</p>
              <ul className="space-y-2 mb-6 text-sm text-stone-600">
                <li className="flex items-center gap-2"><CheckCircle size={16} className="text-emerald-500" /><span>Разговорная практика</span></li>
                <li className="flex items-center gap-2"><CheckCircle size={16} className="text-emerald-500" /><span>Грамматика без лишней теории</span></li>
                <li className="flex items-center gap-2"><CheckCircle size={16} className="text-emerald-500" /><span>Экзамены: стратегии</span></li>
              </ul>
...
const BooksPanel = () => (
  <div className="pb-24 pt-4 px-4">
    <Accordion title="Читать на английском" defaultOpen={true}>
      <MediaRow title="2books.su" desc="Книги в оригинале + перевод." img="https://static.tildacdn.info/tild3539-6535-4239-b735-666530633965/1.jpg" link="https://2books.su/" />
      ...
      <MediaRow title="AnyLang" desc="Чтение, карточки слов." img="https://static.tildacdn.info/tild3564-3631-4036-b636-623266636266/2.png" link="https://anylang.net/ru/books/en" />
...
const VideoPanel = () => (
  <div className="pb-24 pt-4 px-4">
    ...
    <Accordion title="Аудирование">
      <MediaRow title="Listen in English" desc="Уроки по уровням." img="https://static.tildacdn.info/tild3636-3261-4532-b231-626664646132/BA22E78D-3200-4109-8.jpeg" link="https://listeninenglish.com/index.php" />
      <MediaRow title="iSLCollective" desc="Видео‑уроки по фильмам." img="https://static.tildacdn.info/tild3836-3837-4331-b162-623335363239/12.jpg" link="https://en.islcollective.com/english-esl-video-lessons/search" />
    </Accordion>
    <Accordion title="Лексика">
      ...
      <MediaRow title="6 Minute English" desc="Уроки от BBC." img="https://static.tildacdn.info/tild3864-3339-4639-b030-653330343666/IMG_7746.PNG" link="https://www.bbc.co.uk/learningenglish/english/features/6-minute-english" />
    </Accordion>
  </div>
);
...
const PracticePanel = () => (
  <div className="pb-24 pt-4 px-4">
    <Accordion title="Грамматика" defaultOpen={true}>
      <MediaRow title="Bewords.ru" desc="150+ уроков грамматики." img="https://bemat.ru/bewordsgram.jpg" link="https://bewords.ru/" btnText="Открыть" />
      <MediaRow title="Test‑English" desc="Грамматика A1–B2." img="https://static.tildacdn.info/tild3131-3437-4330-a633-393162336665/4.jpg" link="https://test-english.com/grammar-points/" />
      <MediaRow title="Lingust — грамматика" desc="148 уроков." img="https://optim.tildacdn.pub/tild3062-6233-4431-b363-353163363163/-/format/webp/0D4BE37D-2FBF-4950-8.jpeg.webp" link="https://lingust.ru/english/grammar" />
    </Accordion>
    <Accordion title="Перевод предложений">
      <MediaRow title="RU → EN тренажёр" desc="Грамматика через перевод." img="https://static.tildacdn.info/tild6435-3633-4265-b966-313030633165/photo.PNG" link="https://bemat.ru/collect.html" btnText="Открыть тренажёр" />
    </Accordion>
  </div>
);
...
const SpeakPanel = () => (
  <div className="pb-24 pt-4 px-4">
    <Accordion title="Разговорная практика" defaultOpen={true}>
      <MediaRow title="Боб — Английский с ИИ" desc="Поговорит голосом, поправит ошибки." img="https://bemat.ru/bobai.jpg" link="https://t.me/Tobeeng_GPT_bot" btnText="Попробовать" />
      <MediaRow title="HelloTalk" desc="Общение с носителями." img="https://static.tildacdn.info/tild6631-3338-4435-b966-313430333161/_____2.jpg" link="https://www.hellotalk.com/ru" />
      <MediaRow title="Character.AI" desc="Разговор с ИИ." img="https://static.tildacdn.info/tild6435-6666-4139-a237-396664643764/_____3.jpg" link="https://character.ai/" />
    </Accordion>
    <Accordion title="Учить слова">
      <MediaRow title="Bewords" desc="Учите слова на сайте." img="https://bemat.ru/bewordswords.jpg" link="https://bewords.ru/" />
...
export function App() {
  ...
  // Загрузка + планирование уведомлений
  useEffect(() => {
    const saved = localStorage.getItem('bemat_user_v3');
    if (saved) {
      try {
        ...
        const updated: UserState = {
          ...parsed, streak: newStreak, lastVisit: today.toISOString(),
          completedTasks: newCompleted, catFed: newCatFed,
          customTasks: parsed.customTasks || [], streakShownDate: parsed.streakShownDate || '',
          notifHour: parsed.notifHour || 19,
        };
        ...
        if (updated.notificationsEnabled) {
          scheduleNotification(updated.notifHour || 19);
        }
      } catch {
    ...
    // Перепланировать уведомление когда юзер возвращается в приложение
      ...
      if (document.visibilityState === 'visible') {
        ...
        if (s) {
          try {
            ...
            if (p.notificationsEnabled) scheduleNotification(p.notifHour || 19);
          } catch { /* */ }
        }
      }
    };
  ...
  const handleOnboarding = (name: string, goal: UserGoal) => {
    const newUser: UserState = {
      ...
      isOnboarded: true, catFed: false, streakShownDate: '', notifHour: 19,
    };
    setUser(newUser);
  ...
  const closeStreakPopup = () => {
    ...
    if (user) updateUser({ ...user, streakShownDate: new Date().toDateString() });
  };
  ...
  const BackButton = () => (
    <div className="px-4 pt-4">
      <button onClick={handleBack} className="flex items-center gap-2 text-stone-500 font-bold mb-2 hover:text-violet-600">
